  ...
  (function (StockChart) {
    var Chart = (function () {
        ...
        Chart.prototype.initContext = function () {
            //var dpr = Math.max(this.pixelRatio || 1, 1);
            var dpr = 1;
        ...
        Chart.prototype.drawLine = function (line, needStroke) {
            ...
            ctx.setLineWidth(line.size * dpr || 0);
        ...
        Chart.prototype.drawRound = function (round) {
            ...
            ctx.closePath();
            if (round.isStroke) {
                ctx.setStrokeStyle(round.borderColor);
                ctx.stroke();
            }
            if (round.isFill) {
                ctx.setFillStyle(round.fillColor);
                ctx.fill();
            }
        };
        return Chart;
  ...
  (function (StockChart) {
    ...
    var KLine = (function (_super) {
        ...
        KLine.prototype.initialize = function () {
            ...
            this.drawVolumeBar();
            //this.drawAxisYText();
            _super.Actions = this.copyActions();
            this.ctx.draw();
            console.log('canvas updated');
        };
        KLine.prototype.copyActions = function () {
    ...
    function redraw(option) {
        var ctx = option.ctx, textOffsetY = option.textOffsetY || 2,height = option.height, ohlcPrices = option.ohlcPrices,volumes = option.volumes, dataCount = option.dataCount, figureOffsetY = option.figureOffsetY || 20, figureWidth = option.width, volumeHeight = option.volumeHeight,axisTextHeight = option.axisTextHeight || 10 , figureHeight = option.height - volumeHeight - axisTextHeight , figureOffsetHeight = figureHeight - figureOffsetY;
        var len = ohlcPrices.length;
        var count = Math.min(len, dataCount);
        var highPrices = ohlcPrices.map(function (price) {
            return price.h;
        ...
        this.Chart.Actions = ctx.actions;
        ctx.draw();
    }
    function redrawAnimate(option) {
      var ctx = option.ctx, figureWidth = option.width, dataCount = option.dataCount;
      //var dataOffset = option.dataOffset;
      //var unitX = figureWidth / dataCount;
      //var barOffset = dataOffset * unitX;
      var barOffset = option.offset;
      this.Chart.Actions.map(function(v,i,a) {
          if((i + 1) % 3 == 0)
          {
              a[i].data[0].data[0] += barOffset;
              a[i].data[1].data[0] += barOffset;
          }
      });
      ctx.actions = this.Chart.Actions;
      ctx.draw();
    }
    /*
    function updatePriceBar(option) {
    //
        var maxPrice = option.maxPrice, minPrice = option.minPrice, ctx = option.ctx, ohlcPrices = option.ohlcPrices;
        var unitX = this.unitX;
        var unitY = this.unitY = (figureOffsetHeight - 10) / (maxPrice - minPrice);
        var calcY = this.calcY = function (price) {
            return Math.round(figureHeight - Math.abs(price - minPrice) * unitY - 10);
    ...
    */
    ...
    function getBarColor(bars, index) {
        var riseColor = 'rgba(252,63,29,1)', fallColor = 'rgba(85,170,48,1)';
        var openPrice = bars[index].o;
        var closePrice = bars[index].c;
        if (closePrice > openPrice) {
            return riseColor;
    ...
    function drawLine(ctx, line, needStroke) {
        if (needStroke === void 0) { needStroke = true; }
        needStroke = true;
        var dpr = 1;
        ctx.setStrokeStyle(line.color);
    ...
    StockChart.redraw = redraw;
    StockChart.getBarColor = getBarColor;
    StockChart.drawLine = drawLine;
    StockChart.redrawAnimate = redrawAnimate;
  })(StockChart || (StockChart = {}));
  var StockChart;
  (function (StockChart) {
    var TrendLine = (function (_super) {
        ...
        TrendLine.prototype.drawPriceLine = function () {
            ...
            var fillBlock = function () {
                ctx.setStrokeStyle = _this.fillColor;
                ctx.setFillStyle = _this.fillColor;
        ...
        TrendLine.prototype.drawMiddleLine = function () {
            ...
            ctx.beginPath();
            ctx.setStrokeStyle = this.middleLineColor;
